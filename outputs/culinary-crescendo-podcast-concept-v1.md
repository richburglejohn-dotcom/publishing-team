...
- **Episode 1's script is restored to Baron's first-person voice** (recovered from git history, the pre-2026-08-22-rewrite version) — see `outputs/podcast-episode-1-baron-script-v1.md`. The already-produced author-voice audio was superseded and **has now been regenerated (2026-09-12)**: `outputs/podcast-ep1-baron-origin-FULL-v2.mp3` (9:15), via ElevenLabs in the "Baron Vieux American" voice. The 5000-char single-call limit rejected a one-shot attempt (not charged), so it was split into 3 parts and concatenated with short gaps at the seams. The old author-voice audio is kept for history at `outputs/podcast-ep1-baron-origin-FULL-v1-authorvoice-historical.mp3`. **Still open:** the three social clips (`podcast-ep1-clipA/B/C`) were cut from the old author-voice audio and need re-cutting from v2; the spoken disclosure still needs Lejohn to record it and get it spliced onto the front.
- **Episodes 2 (Rampart) and 3 (Melpomene) were never actually rewritten to author-voice** despite this doc's 2026-08-22 note below saying they would be — turns out only Episode 1 got that treatment. Both scripts have been sitting ready in Baron's voice the whole time and can be produced as-is. **Attempted 2026-09-12, blocked on quota:** split both into 2 parts each (same 5000-char limit as Episode 1), all 4 generation calls failed with `quota_exceeded` — the shared ElevenLabs workspace pool (130,810... now showing 190,810 total, quota apparently grew or reset partially) had only 2,266 credits left after Episode 1's 3-part regeneration, not enough for even the smallest remaining chunk (2,287 credits). None of the 4 failed calls were charged. Not retrying against an unchanged quota — holding here until Lejohn tops up credits, same resolution as the earlier Dark Roux Scene 4-7 block.
