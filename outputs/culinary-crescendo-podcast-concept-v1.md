...
- **Episode 1's script is restored to Baron's first-person voice** (recovered from git history, the pre-2026-08-22-rewrite version) — see `outputs/podcast-episode-1-baron-script-v1.md`. The already-produced author-voice audio was superseded and **has now been regenerated (2026-09-12)**: `outputs/podcast-ep1-baron-origin-FULL-v2.mp3` (9:15), via ElevenLabs in the "Baron Vieux American" voice. The 5000-char single-call limit rejected a one-shot attempt (not charged), so it was split into 3 parts and concatenated with short gaps at the seams. The old author-voice audio is kept for history at `outputs/podcast-ep1-baron-origin-FULL-v1-authorvoice-historical.mp3`. **Still open:** the spoken disclosure needs Lejohn to record it and get it spliced onto the front. Social clips: see the dedicated section below — done.
- **Episodes 2 (Rampart) and 3 (Melpomene) were never actually rewritten to author-voice** despite this doc's 2026-08-22 note below saying they would be — turns out only Episode 1 got that treatment. Both scripts have been sitting ready in Baron's voice the whole time. **DONE 2026-09-12:** first attempt blocked on quota (all 4 calls failed `quota_exceeded`, none charged); Lejohn topped up credits and the retry succeeded cleanly. Both produced via ElevenLabs in the "Baron Vieux American" voice, split into 2 parts each (5000-char limit), concatenated with short gaps at the seams: `outputs/podcast-ep2-rampart-FULL-v1.mp3` (7:37) and `outputs/podcast-ep3-melpomene-FULL-v1.mp3` (6:17). **Social clips (A/B/C) for all three episodes: DONE 2026-09-12.** Cut from each episode's finished full audio: `outputs/podcast-ep{1,2,3}-clip{A,B,C}-{coldopen,lesson,cta}-v{2,1,1}.mp3` (Episode 1 is v2 since its audio changed; 2 and 3 are v1, first cut). No manual listening available, so used the same method as the original Episode 1 clips (per this doc's 2026-08-24 status note above): word-position estimation against each TTS chunk's known duration for an initial timestamp, cross-checked against `ffmpeg silencedetect` pause points in the actual audio and snapped to the nearest one within 1.5s (kept the word-position estimate, lightly padded, when nothing fell within that window). Verified both edges of all 9 clips via `volumedetect` on the first/last 0.3s for abrupt/hot cutoffs — all landed in quiet regions, no sign of a mid-word cut. Durations: Ep1 A/B/C = 10.9s/22.8s/24.0s, Ep2 = 15.3s/21.2s/17.6s, Ep3 = 14.3s/29.3s/21.9s — all within each script's own ~15-30s target. Episode 1's old clips (from the superseded author-voice audio) kept for history as `*-v1-authorvoice-historical.mp3`.

**Visuals paired — 2026-09-12 — DONE.** All 9 clips now exist as postable vertical (1080x1920) video: `outputs/podcast-ep{1,2,3}-clip{A,B,C}-{coldopen,lesson,cta}-vertical-v1.mp4`. Used the show's own cover art (`outputs/podcast-apothecary-sessions-cover-art-v1.png`) uniformly across all 9 rather than per-episode character art — Rampart has a locked portrait (`references/rampart-portrait-v1.jpg`) but Melpomene has none at all, and per-episode art for 2 of 3 characters with a gap for the third would look inconsistent. First attempt used a blurred-pad background (the technique used elsewhere in this project for photo-rich source images) but the cover art is mostly dark/black at the edges, so the padding read as broken black bars rather than intentional design — switched to scale-and-crop-to-fill instead, which suits this centered-logo, dark-background image much better (confirmed visually).
