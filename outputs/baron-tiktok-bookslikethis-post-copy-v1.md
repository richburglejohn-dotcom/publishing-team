# Baron Vieux — "Books Like This" TikTok Post Copy
### Created: 2026-07-28 · Updated: 2026-07-31
**Video (current, ready to post):** `outputs/baron-tiktok-bookslikethis-v5.mp4` — v4 with two fixes: (1) the synthetic espeak-ng voiceover is removed entirely — author decided against ElevenLabs and doesn't want a synthetic narration track, so the video now runs on the clips' own native ambient audio only; (2) the CTA book-cover prop's garbled author byline ("BAYEEN JEREOJIER") is corrected to read "Lejohn Richburg" via a direct compositing fix on the source clip. Runtime ~35.7s. Captions are unchanged from v4 — these clips are quiet/ambient, not narrated, so the on-screen text cards are still doing the storytelling work here (unlike the chef's-jacket line, there are no native captions to clash with).
**Video (v4, superseded — has synthetic voiceover and the garbled author name):** `outputs/baron-tiktok-bookslikethis-v4.mp4` — v3 trimmed from ~44s to ~35.7s for TikTok pacing.
**Video (v3, 5 scenes + narration, untrimmed):** `outputs/baron-tiktok-bookslikethis-v3.mp4` — v2 plus a 5th scene ("The Resolution," supplied 2026-07-30) inserted between the Turn and the CTA, giving the "friends refuse to let it die" / "sober, free, finally living" beat real footage instead of a text-only card. Runtime ~44s.
**Video (v2, narration, 4 scenes only):** `outputs/baron-tiktok-bookslikethis-v2.mp4` — v1 plus a narration track for the script's three VOICE lines, mixed under the ambient audio (ducked) with the caption timing re-synced to match.
**Video (v1, captions-only, no narration):** `outputs/baron-tiktok-bookslikethis-v1.mp4` — stitched from 4 clips per `outputs/social-content-book1-deepdive-v1.md`, TikTok Script 4 ("Books Like This" Trend Format)

**IMPORTANT — placeholder narration:** the voiceover (v2 and v3) is generated with `espeak-ng` (a robotic offline text-to-speech engine), not Baron's ElevenLabs voice clone. This environment has no ElevenLabs API key and its network proxy blocks outbound calls to api.elevenlabs.io, so the real cloned-voice narration couldn't be generated here. The espeak-ng track is a placeholder to get the timing/pacing right — swap it for a real ElevenLabs render of the same three lines before this goes out.

Source clips (v3, in order):
1. Hook — Baron at the bar, black vest, looking down at the book
2. The Setup — shipping-container exterior at dusk → Baron's portrait, arms crossed
3. The Turn — Baron at the bar, hands clasped, looking down
4. **The Resolution** (new) — Baron at the bar, eyes closed, exhaling — upscaled from a 720x1280 preview export to match the others (1080x1920), so it reads slightly softer than the rest
5. CTA — book placed on a candlelit wooden table

Captions burned in per the script's text cues (branded Playfair Display / parchment-on-scrim treatment, matching the Elixir/Origin/Flashback/Family Meal videos):
- "Books that will absolutely wreck you (in a good way):"
- "A sous chef and a blues singer build a restaurant out of a shipping container in New Orleans."
- "They fall in love. They build the dream."
- "Then the dream almost destroys them — burnout, addiction, a business on the edge of collapse."
- "Then their friends refuse to let it die."
- "They come out the other side — sober, free, finally living."
- "Culinary Crescendo: A Symphony of Love in the Big Easy"
- "Read it. Then tell me you're okay."

---

## TikTok caption

> Books that will absolutely wreck you (in a good way). 📖🔥
>
> Culinary Crescendo: A Symphony of Love in the Big Easy — link in bio.

**Hashtags:** #BooksThatWreckYou #BookTok #CulinaryCrescendo #BaronVieux #NewOrleans #ChefLife #RecoveryStory #FoundFamily #FuturIQ

---

## Notes
- Fulfills the 2026-07-27 TikTok slot (Script 4). TikTok is posted manually by Lejohn.
- v2 narration lines (VOICE beats only — the other text cards are on-screen text with no spoken line, per the script): "A sous chef and a blues singer build a restaurant out of a literal shipping container in New Orleans." / "Then the dream almost destroys them: burnout, addiction, a business on the edge of collapse." / "And they come out the other side, sober, free, and finally living instead of just surviving."
- **Flag:** in the CTA clip's book-cover prop, the author byline renders as "BAYEEN JEREOJIER" instead of "Lejohn Richburg III" — an AI text-rendering error baked into the source video. It's partially covered by the CTA caption box but still visible in the corner. Not fixed here since it would require re-generating or re-compositing the source clip; flagging for your call on whether it's worth a fix pass before this goes out.
- Clip originally sent as 4 files; a missing "Scene 2 — The Setup" clip was requested and supplied separately, then slotted into position 2.
