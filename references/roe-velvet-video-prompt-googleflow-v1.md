...
**Trimmed (2026-09-04):** the 10.01s version had her repeat the line a second time from ~8.0-9.0s (padding filled by Flow to reach the extended 10s duration) before the true final smile — caught via the stitched full sequence at 0:46-0:48. Found the exact repeat onset (between 8.0-8.15s) by cross-checking ffmpeg's silencedetect audio gaps against frame-by-frame mouth movement, since no transcription tool was available to just read the words. Trimmed to end at 8.0s (right on the first, genuine smile), dropping the repeat entirely — 10.01s → 8.00s. Full sequence re-stitched: 65.46s → 63.46s.
