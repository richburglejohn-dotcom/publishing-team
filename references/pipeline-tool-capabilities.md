...
**Metricool — connected 2026-09-05, not yet usable.** Solves the YouTube Short-vs-regular-upload gap Buffer can't (`youtubeData.type`: `"video"` or `"short"`, an explicit toggle) — but no social networks are linked inside Metricool yet (`getBrandSettings` returns `networksData: {}`). Needs Lejohn to link YouTube (and any other networks) from Metricool's own dashboard before this is usable. No Community-post support found here either — same gap as Buffer, this isn't scheduler-specific.
