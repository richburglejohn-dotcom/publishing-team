...
- **Instagram:** Baron.Vieux + FuturIQ profiles. Same — real API access, not browser. **Note (2026-09-05): Baron.Vieux specifically can now also be posted directly by Claude via Windsor.ai** (see below) — Koda's IG access to that one profile is now redundant with Claude's, not the only path. FuturIQ IG still Koda-only.
...
**Instagram — LIVE as of 2026-09-05, no Koda hop needed.** Claude can publish directly to Instagram via the Windsor.ai connector (`create_video_post`/`create_image_post`/`create_carousel_post`/`create_story` against account `17841442720360055`, "Baron Vieux (baron.vieux)"). Getting here took three separate fixes, all now done: (1) Windsor.ai write actions enabled account-wide (Settings → API Access → "Enable write actions for Claude, ChatGPT & API"), (2) the Instagram connection reauthorized with `instagram_content_publish` permission (it was originally connected without it — Windsor returned a clear permission error until this was fixed), (3) Windsor.ai enabled for this chat specifically (`enabledInChat`, separate from account-level connection — toggled per-session, not just per-account). First real post through this path: Roe Velvet Reel, media id `18122691977486654`, published 2026-09-05. **For any future IG post, just call it directly — no more staging-for-Koda needed for this specific account.**

**Facebook still has no equivalent path.** Both Windsor.ai's `facebook` connector and the Meta Ads MCP tool only expose ad/boost actions (`boost_post`, `create_ad`, etc.), no organic page-post action; whatever token Koda's FB access uses has a publish scope neither of Claude's connectors currently has. Until a connector with `pages_manage_posts`-equivalent scope is set up for Claude, FB still needs Koda or Lejohn.

**Metricool — connected 2026-09-05, YouTube linked and usable.** Solves the YouTube Short-vs-regular-upload gap Buffer can't (`youtubeData.type`: `"video"` or `"short"`, an explicit toggle). Lejohn linked the YouTube channel (`UCtWwedsXe1zQ1cGGKb0z9QA`) same day — `createScheduledPost` calls need `blogId: "6854241"` (the Metricool brand id) and `providers: [{"network":"youtube"}]`. Not yet used for an actual post — the next YouTube upload that needs to land as a regular video (not a Short) should go through this instead of Buffer. Only YouTube is linked so far; other networks would need linking separately in Metricool's dashboard if ever wanted there. No Community-post support found here either — same gap as Buffer, this isn't scheduler-specific.
