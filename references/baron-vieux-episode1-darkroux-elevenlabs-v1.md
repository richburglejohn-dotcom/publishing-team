...
**Gap found, then corrected: Scenes 1-3 (generated 2026-09-08) were never saved to the repo as individual files** — they existed only as completed nodes on the flow canvas, and no session_id was ever logged anywhere to retrieve that original run's media directly. Flagged to Lejohn; he chose to regenerate rather than lose the episode. Started re-running the original flow nodes (`2wLfoFaoU6AB5Tol7dxD` Scene 1, `bGIuIzJJLqZYBMuQCRfu` Scene 2) — Scene 1's first take hallucinated invented garbled embroidery on the apron, fixed via a strengthened no-invented-text prompt and regenerated clean; Scene 2's regeneration was in flight when Lejohn asked whether the clips could instead be pulled from the already-posted teaser.

**They could.** `outputs/darkroux-teaser-v1.mp4` (posted 2026-09-08, still in the repo) is explicitly "Scenes 1-3 of Episode 1 stitched together" per its own doc (`outputs/darkroux-teaser-post-copy-v1.md`) — the actual original "best result of the whole week" footage, not a fresh take. Split it at the 10.01s/20.02s boundaries (matching each scene's individual ~10.01s generation length) into `outputs/baron-vieux-darkroux-scene{1,2,3}-v1.mp4`, verified each against the locked reference — clean cut at the Scene 1/2 boundary (confirmed via frame check at t≈9.8→10.2), Scene 3 correctly shows the color progressing from golden to deep mahogany, connecting cleanly into Scene 4's "dark bubbling roux." The regenerated Scene 1 take (which had also passed verification) was superseded by this real original and discarded. The in-flight Scene 2 regeneration was superseded too — its credits (~6,105) were spent before the teaser was found, an avoidable cost from not checking the teaser first.

## Full 7-scene stitch check — 2026-09-12 — PASSED (with one flagged, accepted note)

All 7 scenes normalized (24fps, matched audio format) and concatenated in order into `outputs/baron-vieux-episode1-darkroux-full-v1.mp4` (~70s). Checked each of the 6 cut points for visible jumps per the verification protocol:

- **Cuts 1-4 (Scenes 1→2→3→4→5): clean.** Same kitchen, same skillet, natural stage progression at every cut — camera angle changes match what each scene's own script called for (medium shot → overhead → macro → side angle), not drift.
- **Cut 5 (Scene 5→6): clean, expected.** Skillet moves from stovetop to a wooden cutting board with a thyme sprig — matches Scene 6's own script exactly ("thyme sprig on the cutting board beside it").
- **Cut 6 (Scene 6→7): flagged.** The cutting board and thyme sprig from Scene 6 are gone in Scene 7 — back to a plain counter — even though Scene 7's own script says "same beauty shot holds." Not a character/text/identity defect (Baron absent from both as scripted, kitchen and roux color still match) — a staging continuity wobble between two independently-generated clips. Flagged to Lejohn per standing discipline rather than silently accepted or silently patched; he chose to accept it as-is (same tolerance as other scene-to-scene framing variance in this project) rather than spend another ~6,105 credits regenerating Scene 7.

**Episode complete: all 7 scenes generated/recovered, verified, and stitched.**

## Narration mix — 2026-09-12 — DONE

The original 2026-09-08 narration audio had the same recoverability gap as the video scenes — generated on the flow, never saved to the repo, no logged session_id. Same fix: re-ran the 7 existing TTS nodes on the flow (cheap, ~$0.11 for all 7 last time) rather than trying to recover the unrecoverable. Ran all 7 at once first; 2 (lines 1 and 4) hit the workspace's 5-concurrent-request cap and failed cleanly (no charge per the error), retried those 2 once the others cleared — all 7 completed. Verified line 2 and line 4 weren't an accidental duplicate (identical duration by coincidence, different audio — confirmed via file hash).

Mixed: each narration line padded with silence to exactly match its scene's duration (a small ~0.3s lead-in, narration at full volume), concatenated into one 70.16s track, then mixed under the video's existing baked-in ambient/sizzle audio (ambient lowered to 35% so narration reads clearly on top, not replaced). Saved as `outputs/baron-vieux-episode1-darkroux-full-v2.mp4` — `v1` (silent b-roll, ambient audio only) kept for reference, not deleted.
