...
**Result (2026-08-21):** art now exists — filed as `outputs/apothecary-family-meal-anchor-v1.png`. QA against the checklist above: Baron's glasses and gray coat held; Ghost correctly reads as a plain-shirted outsider rather than kitchen whites; the crew's race mix matches the bible (Black/white split correct across the five). Not independently re-verified against every single trait (e.g. Rampart's exact scar/build call) at full resolution, but overall a usable result — no need to fall back to the solo-portrait composite for this one.
